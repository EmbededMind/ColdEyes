...
public:
	...
	static CSerialPort* GetInstance(int portnr)
	{
		if (portnr == COM_KB)
		{
			static CSerialPort ComKB;
			return &ComKB;
		}
		else if(portnr == COM_103)
		{
	...
	BOOL        ConfigPort(UINT portnr = 1, 
				UINT baud = 19200,
	            char parity = 'N', 
				UINT databits = 8, 
				UINT stopbits = 1,
				DWORD dwCommEvents = EV_RXCHAR | EV_CTS, 
				UINT writeBufferSize = 512,
	...
	BOOL        InitPort(IDataHandler* pDataHandler, UINT portnr = 1, UINT baud = 19200,
				char parity = 'N', UINT databits = 8, UINT stopsbits = 1,
				DWORD dwCommEvents = EV_RXCHAR | EV_CTS, UINT nBufferSize = 512,
	...
	void        AttachWindow(CWnd* pOwnerWin);
	void        AttachDataHandle(IDataHandler* pDataHandler);
